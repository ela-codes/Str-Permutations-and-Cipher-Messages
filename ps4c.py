...
class SubMessage(object):
    ...
    def build_transpose_dict(self, vowels_permutation):
        ...
        for i in range(len(VOWELS_LOWER)):
            transposed_dict[VOWELS_LOWER[i]] = vowels_permutation[i]
            transposed_dict[VOWELS_UPPER[i]] = vowels_permutation[i].upper()

        for i in range(len(CONSONANTS_LOWER)):
            transposed_dict[CONSONANTS_LOWER[i]] = CONSONANTS_LOWER[i]
            transposed_dict[CONSONANTS_UPPER[i]] = CONSONANTS_UPPER[i]
            ...
            if len(i) > 1:
                for j in i:
                    if not j.isalpha():
                        ...
                        encrypted_word += transpose_dict[j]
            else:
                if not i.isalpha():
                        encrypted_word += i
                else:
                    encrypted_word += transpose_dict[i]
...
class EncryptedSubMessage(SubMessage):
    ...
    def decrypt_message(self):
        ...
        for curr_perm in permutation_list: # check each permutation in list
            test_transpose_dict = self.build_transpose_dict(curr_perm)
            test_transpose_word = self.apply_transpose(test_transpose_dict)
            count_real_words = len([i for i in test_transpose_word.split() if is_word(self.valid_words, i) is True])

            if count_real_words > max_count_of_real_words: 
                max_count_of_real_words = count_real_words
                decrypted_msg  = test_transpose_word
...
if __name__ == '__main__':
    ...
    message2 = SubMessage("What's your name?")
    permutation2 = "uoeai"
    ...
    print("Expected encryption:", "Whut's yair nomu?")
    ...
    message3 = SubMessage("My name's Angel.")
    permutation3 = "eoaui"
    ...
    print("Expected encryption:", "My nemo's Engol")
